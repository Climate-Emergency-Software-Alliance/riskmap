...
export class MapLayers {
  ...
  fetchIcon = (type, level, isPartnerIcon, isSelected = false) => {
    if (isPartnerIcon) {
      return isSelected ? `assets/icons/onselect/${type}_partnericon_${level}_select.svg` : `assets/icons/${type}_partnericon_${level}.svg`;
    }
  ...
  markerClickHandler(e, feature, cityName, map, togglePane) {
    ...
    const popup = document.querySelector('report-popup').au.controller.viewModel;
    if (popup) {
      popup.setReport(feature);
      popup.open();
    }
    ...
    layer.on({
      click: e => {
        ...
        if (!self.selected_gauge) {
          ...
          e.target.setIcon(
            self.mapIcons.gauge_normal(
              self.gaugeIconUrl(e.target.feature.properties.observations[e.target.feature.properties.observations.length - 1].f3)
            )
          );
          togglePane('#infoPane', 'hide', false);
          self.selected_gauge = null;
  ...
  getFloodDepthFeet(depth) {
    return (depth * 2) / 33.33;
  }

  _getFloodSevearity(depth) {
    const depthInFeet = this.getFloodDepthFeet(depth);

    if (depthInFeet < 0.5) {
      return 'normal';
    } else if (depthInFeet >= 0.5 && depthInFeet <= 1) {
      return 'medium';
    } else if (depthInFeet > 1) {
      return 'high';
    }
  }
